...
def create_model(model_name):
    if model_name == 'EB_Fusion':
        return EB_Fusion(num_channels=19, num_frequencies=49).to(device)
    if model_name == 'EB':
        return EB(num_channels=19, num_frequencies=49).to(device)
    if model_name == 'FB_Fusion':
        return FB_Fusion(num_channels=19, num_frequencies=49).to(device)
    if model_name == 'FB':
        return FB(num_channels=19, num_frequencies=49).to(device)
    if model_name == 'Only_RC':
        return Only_RC(num_channels=19, num_frequencies=49).to(device)
    if model_name == 'Only_SAPL':
        return Only_SAPL(num_channels=19, num_frequencies=49).to(device)
    if model_name == 'No_Prune':
        return No_Prune(num_channels=19, num_frequencies=49).to(device)
    else:
        return None
# 如果有 GPU 可用，则使用 GPU
