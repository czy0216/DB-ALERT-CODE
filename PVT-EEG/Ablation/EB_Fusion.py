...
class EB_Fusion(torch.nn.Module):
    def __init__(self, num_channels, num_frequencies):
        super(EB_Fusion, self).__init__()
        ...
        self.out = torch.nn.Sequential(
            torch.nn.Linear(in_features=self.chan_freq_branch.out_size, out_features=16),
            nn.ReLU(),
    ...
    def forward(self, x):
        # 双分支
        chan_freq_out, _ = self.chan_freq_branch(x)

        out = self.out(chan_freq_out)
        return out

# x = torch.rand(size=(200, 17, 49)).cuda()
# model = EB_Fusion(num_channels=17, num_frequencies=49).cuda()
# # summary(model, input_size=(17, 63))
# x = model(x)
# print(x.shape)
