...
from model import EEG_Alert  # 论文架构
...
def vis_result(model_name, f):
    ...
    channel_names = np.array(['FT7', 'FT8', 'T7', 'T8', 'TP7',
                              'TP8', 'CP1', 'CP2', 'P1', 'Pz',
                              'P2', 'PO3', 'POz', 'PO4', 'O1',
                              'Oz', 'O2'])
    for i in range(23):
...
def create_model(model_name, num_channels, num_frequencies):
    if model_name == 'DeepConvNet':
        return DeepConvNet(num_channels=17, num_samples=1600, num_classes=2).to(device)
    if model_name == 'ShallowConvNet':
        return ShallowConvNet(num_channels=17, num_samples=1600, num_classes=2).to(device)
    if model_name == 'EEGNet':
        return EEGNet(num_channels=17, num_samples=1600, num_classes=2).to(device)
    if model_name == 'ResNet':
        return ResNet().to(device)
    if model_name == 'VisionTransformer':
        return VisionTransformer().to(device)
    if model_name == 'temp':
        return Temp_model(num_channels=num_channels, num_frequencies=num_frequencies).to(device)
    else:
        return EEG_Alert(num_channels=num_channels, num_frequencies=num_frequencies).to(device)


def cal_avg_results(X, Y, model):
    all_results = {
        'Accuracy': [],
        'Precision': [],
        'Recall': [],
    ...
    for i in range(n_splits):
        # 加载预训练模型
        model = create_model(model, num_channels, num_frequencies)  # 假设create_model()返回模型结构
        model.load_state_dict(torch.load(f'Control/best_model_{i + 1}.pth', weights_only=True))
        model.eval()
        # 数据加载
        dataset = EEGData(X, Y)  # 假设EEGData是您定义的数据集类
        loader = DataLoader(dataset, batch_size=885, shuffle=False)
        # 存储真实标签和预测标签
        all_labels = []
        all_predictions = []
...
def train_kf(X, Y, model_name, path_name):
    ...
    kf = KFold(n_splits=n_splits, shuffle=True)  # LOSOCV
...
path = '' # 路径修改
for name in ['ShallowConvNet', 'EEGNet', 'DeepConvNet']:
    print(f'==========Model Name {name}==========')
    train_kf(X, Y, name,path)
for name in [
    'VisionTransformer',
    'ResNet'
]:
...
for f in range(17):
    train_kf(X, Y, 'model', 'Control')
    vis_result(model_name='model', f=f)

"""对X Y不同类别进行可视化"""
print(X.shape, Y.shape)
h_x = X[Y == 0]
l_x = X[Y == 1]
print(h_x.shape, l_x.shape)
h_x = np.mean(h_x, axis=0)
l_x = np.mean(l_x, axis=0)
print(h_x.shape, l_x.shape)
# 创建一个2x1的子图（两列并列显示）
fig, axes = plt.subplots(1, 2, figsize=(12, 6))

# 绘制 h_x 的热力图
sns.heatmap(h_x[:,], ax=axes[0], cmap='viridis', cbar=True)
axes[0].set_title('Heatmap of h_x')  # 设置标题

# 绘制 l_x 的热力图
sns.heatmap(l_x[:,], ax=axes[1], cmap='viridis', cbar=True)
axes[1].set_title('Heatmap of l_x')  # 设置标题
