...
"""DeepConvNet ShallowConvNet EEGNet 将原始脑电信号输入"""
class DeepConvNet(nn.Module):
    def __init__(self, num_channels, num_samples, num_classes):
        ...
        self.dense = nn.Linear(200 * 96, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.pooling3(x)
        x = F.relu(self.batch_norm4(self.conv5(x)))
        x = self.pooling4(x)
        x = self.flatten(x)
        x = self.dense(x)
        return x
...
class ShallowConvNet(nn.Module):
    def __init__(self, num_channels, num_samples, num_classes):
        ...
        self.flatten = nn.Flatten()
        self.dense = nn.Linear(40 * 224, num_classes)
...
class EEGNet(nn.Module):
    def __init__(self, num_channels, num_samples, num_classes):
        ...
        self.first_conv = nn.Conv2d(1, 16, (1, num_samples // 2), padding=(0, num_samples // 4))
        ...
        self.flatten = nn.Flatten()
        self.dense = nn.Linear(32 * 48, num_classes)
...
class VisionTransformer(nn.Module):
    def __init__(self, num_classes=2, emb_size=64, depth=1, num_heads=1, ff_hidden=64*4):
        ...
        self.transformer_blocks = nn.Sequential(*[TransformerEncoderBlock(emb_size, num_heads, ff_hidden) for _ in range(depth)])
